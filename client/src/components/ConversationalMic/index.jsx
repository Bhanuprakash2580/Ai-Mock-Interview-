  ...
  return (
    <div className="cm-container">
      ...
      <div className="cm-status">
        {isListening && !displayText && <p className="cm-status-listening">Listening...</p>}
        {isListening && displayText && !autoSubmitCountdown && <p className="cm-status-listening">Hearing you...</p>}
        {autoSubmitCountdown && (
          <p className="cm-status-countdown">
            Submitting in {autoSubmitCountdown}s...{' '}
            <button className="cm-keep-talking-btn" onClick={() => { clearSilenceTimer(); startListening(); }}>
              Keep talking
            </button>
          </p>
        )}
        {!isListening && displayText && <p className="cm-status-done">Done</p>}
        {!isListening && !displayText && <p className="cm-status-ready">Ready to listen</p>}
      </div>

      {displayText && (
        <div className="cm-transcript-box">
          <span className="cm-transcript-final">{finalText}</span>
          {liveText && <span className="cm-transcript-interim">{liveText}</span>}
        </div>
      )}
